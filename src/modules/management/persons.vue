<template>
  <q-card bordered class="border-radius-10">
    <q-toolbar>
      <q-toolbar-title>{{ $t('sidebar.person') }}</q-toolbar-title>
        ...
        <q-markup-table v-if="homeId">
          <thead>
            <q-tr>
              ...
              <q-th>{{ $t('fields.passport') }}</q-th>
              <q-th>{{ $t('fields.phone') }}</q-th>
              <q-th>{{ $t('fields.work') }}</q-th>
          ...
          <tbody>
            <q-tr v-if="list&&list.length>0" v-for="(item, index) in list" :key="`item-${index}`">
              ...
              <q-td class="text-center">{{ item.f_i_o }}</q-td>
              <q-td class="text-center">{{ item.passport }}<br>{{ item.pin_fl }}</q-td>
              <q-td class="text-center">{{ item.phone }}</q-td>
              <q-td class="text-center">{{ item.work }}</q-td>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      this.loading = true
      await PersonService.list(this.homeId).then(res => {
        this.list = res.data
      }).catch(e => {
        this.$q.notify({
    ...
    async deleteForm(id) {
      this.loading = true
      await PersonService.delete(id).then(res => {
        this.getList()
        this.$q.notify({
    ...
    async save() {
      ...
      await PersonService.save(this.form, this.create).then(res => {
        this.modal = false
        this.getList();
        this.$q.notify({
